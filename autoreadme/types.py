...
class section:
    ...
    def __init__(self, name: dict, prompt: dict):
        self.name, self.prompt = name, prompt['general_description']
        self.subsections = prompt['subsections']
...
class AutodocReadmeConfig:
    ...
    Attributes:
        sections (dict[key, dict]): A dictionary with keys as markdown title and values as dictionary with subsections. EG.
            {'Description': {
                'general_instruction': 'Take into account that this repo is a backend for a chat agent dedicated to answer questions',
                'subsections': {'Kafka': 'Describe how this repo uses Kafka to integrate services'}}
            }
            All dictionaries must have the structure:
            {
                "title": {
                    'general_instruction': 'general prompt',
                    'subsections': {
                        'subtitle1': 'prompt1',
                        'subtitle2': 'prompt2',
                        'subtitle3': 'prompt3'...
    ...
    def __init__(self, sections: dict):
        self.sections = [section(name, prompt) for name, prompt in sections.items()]
