...
from autoreadme.utils.file_utils import (
    get_file_name,
    github_file_url,
    github_folder_url,
)
from autoreadme.utils.llm_utils import get_tokenizer, models
from autoreadme.utils.traverse_file_system import traverse_file_system
from autoreadme.utils.llm_utils import get_llm_encoder
from autoreadme.config.prompts import (
...
def process_repository(
    ...
    def process_file(process_file_params: ProcessFileParams):
        ...
        if "llama" in model.name.lower() or "gemma" in model.name.lower():
            ...
            encoding = get_llm_encoder(model.name)
    ...
    def process_folder(process_folder_params: ProcessFolderParams):
        ...
        for x in contents_list:
            if not process_folder_params.should_ignore(x.as_posix(), params.ignore):
