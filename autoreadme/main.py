"""CLI interface for autoreadme project."""
...
def main():  # pragma: no cover
    ...
    `python -m autoreadme` and `$ autoreadme `.
    """
    ...
    name = questionary.text(
        message="Project Name?[Example: autoreadme]"
    ).ask()
    project_root = questionary.path(
        message="Project Root?[Example: ./autoreadme/]",
        only_directories=True,
        default=f"./{name}/",
    ).ask()
    project_url = questionary.text(
        message="Project URL?[Example: "
        + "https://github.com/username/autoreadme]",
        validate=url_validator,
    ).ask()
    output_dir = questionary.path(
        message="Output Directory?[Example: ./output/autoreadme/]",
        only_directories=True,
